...
#define  pr_info  pr_err  //for debug
...
int s3c_sdhc_pin_config(int peripheral, int flags)
{
	...
	switch (peripheral) {
	case PERIPH_ID_SDMMC0:pr_debug("PERIPH_ID_SDMMC0!\n");
		start = S3C64XX_GPG(0);
		end  = S3C64XX_GPG(6);
		func = S3C_GPIO_FUNC(0x2);
		break;
	case PERIPH_ID_SDMMC1:pr_debug("PERIPH_ID_SDMMC1!\n");
		start = S3C64XX_GPH(0);
		end = S3C64XX_GPH(5);
		func = S3C_GPIO_FUNC(0x2);
		break;
	...
	if(peripheral == PERIPH_ID_SDMMC1) {/* for cmd/clk pins*/
		gpio_set_cfg(S3C64XX_GPG(6),  3);pr_debug("PERIPH_ID_SDMMC1 config cd\n");
		gpio_set_pull(S3C64XX_GPG(6), S3C_GPIO_PULL_UP);
	}
	return 0;
}
